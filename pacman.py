...
Mods:
1. George Rudolph 24 Jan 2022 put main code in function main().
2. George Rudolph 24 Jan 2022 fixed import order:
   builtin modules before user-defined modules
3. "static" variables are class variables in Python
4. George Rudolph 24 Jan 2022 getAndResetExplored changed to class method also changed to snake case.
"""
import sys
import types
import time
...
class GameState:
    ...
    # class variable keeps track of which states have had getLegalActions called
    explored = set()

    @classmethod
    def get_and_reset_explored(cls):
...
def loadAgent(pacman: str, nographics: bool) -> Any:
    """
    Load the requested agent class from a module.
    ...
    if pacman == "RandomGhost":
        from ghostAgents import RandomGhost
        return RandomGhost
    if pacman == "DirectionalGhost":
        from ghostAgents import DirectionalGhost
        return DirectionalGhost

    # Then look through all pythonPath Directories for the right module
